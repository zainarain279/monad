  ...
  while (retries <= maxRetries) {
    try {
      ...
      if (retries > maxRetries) {
        console.error(`❌ Error fetching token balance for ${token.name} after ${maxRetries} attempts: ${error.message}`.red);
        return { raw: ethers.BigNumber.from(0), formatted: "0" };
      }

      console.log(`⚠️ Error fetching token balance for ${token.name} (attempt ${retries}/${maxRetries}): ${error.message}`.yellow);
...
async function approveTokenIfNeeded(wallet, token, amount, routerAddress) {
  ...
  try {
    ...
    if (allowance.lt(amount)) {
      ...
      console.log(`✅ Token ${token.name} has already been approved`.green);
    }
    return true;
  } catch (error) {
...
async function swapTokens(wallet, tokenA, tokenB, amountIn, isToMON = false) {
  try {
    ...
    console.log(`🔄 Swapping ${formattedAmountIn} ${tokenA.name} → ${tokenB.name}`.magenta);
...
async function swapNativeToken(wallet, targetToken, amountIn) {
  try {
    ...
    console.log(`🔄 Swapping ${ethers.utils.formatEther(roundedAmountIn)} MON → ${targetToken.name}...`.magenta);
    ...
    console.error(`❌ Error swapping MON → ${targetToken.name}:`.red, error.message);
    return false;
  }
}
...
async function swapMonToToken(wallet, token) {
  try {
    ...
    console.error(`❌ Error swapping MON to ${token.name}: ${error.message}`.red, error.message);
    return false;
  }
}
  ...
  try {
    ...
    for (let i = 0; i < cycles; i++) {
      ...
      if (i < cycles - 1) {
        ...
        console.log(`⏱️ Waiting for ${Math.floor(cycleDelay / 1000)} seconds before next cycle...`.cyan);
